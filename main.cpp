...
status::ElementOrientation playerOrientation = status::ElementOrientation::DOWN;
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    ...
    character::Character player("assets/Sprout Lands - Sprites - premium pack/Characters/Basic Charakter Spritesheet.png", renderer);
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    {
        dy -= 1.0f;
        playerOrientation = status::ElementOrientation::UP;
    }
    if (keys[SDL_SCANCODE_S])
    {
        dy += 1.0f;
        playerOrientation = status::ElementOrientation::DOWN;
    }
    if (keys[SDL_SCANCODE_A])
    {
        dx -= 1.0f;
        playerOrientation = status::ElementOrientation::LEFT;
    }
    if (keys[SDL_SCANCODE_D])
    {
        dx += 1.0f;
        playerOrientation = status::ElementOrientation::RIGHT;
    }
    ...
    float speed = 80.0f; // píxeles por segundo
    positionX += dx * speed * deltaTime;
    positionY += dy * speed * deltaTime;
    ...
    SDL_FRect srcrect = {
        ...
        spriteYPosition(playerOrientation),
        SPRITE_WIDTH,
        SPRITE_HEIGHT};
    ...
    if (SDL_HasRectIntersectionFloat(&dstrect, &dstRect2))
    {
...
static float spriteIndexIdleOrMovement()
{
    ...
    {
        int rawIndex = (SDL_GetTicks() / 200) % 4; // más rápido: 200 ms por frame
...
{
    /* SDL limpia window/renderer automáticamente */
}
